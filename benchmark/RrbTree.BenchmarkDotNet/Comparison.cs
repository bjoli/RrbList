...
public class RrbListBenchmarks
{
    ...
    {
        var list = _rrbList;
        foreach (var idx in _randomIndexes)
        {
            list = list.SetItem(idx, 999);
        }
        return list;
    }
    ...
    public RrbList<int> Slice_RrbList() => _rrbList.Slice(_middleIndex / 2, N/4);
    ...
    [BenchmarkCategory("Slice")]
    public List<int> Slice_List() => _list.GetRange(_middleIndex / 2, N/4);
    
    // --- 8. MERGE / ADD RANGE ---
    [Benchmark(Description = "RrbList.Merge")]
    [BenchmarkCategory("Merge")]
    public RrbList<int> Merge_RrbList() => _rrbList.Merge(_rrbChunk);
    
    [Benchmark(Description = "RrbListUnbalanced.Merge")]
    [BenchmarkCategory("Merge")]
    public RrbList<int> Merge_RrbListUnbalanced() => _rrbUnbalanced.Merge(_rrbChunk);
