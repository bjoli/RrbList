...
public sealed partial class RrbList<T> : ICollection<T> where T : notnull
{
    ...
    public RrbList<T> Sli2ce(int start, int count)
    {
        if (start < 0 || start + count > Count) throw new ArgumentOutOfRangeException();
        if (count == 0) return Empty;
        if (count == Count) return this;

        // 1. Short Slice Optimization (Crucial for performance)
        if (count <= Constants.RRB_BRANCHING * 2)
        {
            ...
            var tailCount = count - Constants.RRB_BRANCHING;
            var newTailItems = new T[tailCount];

            CopyTo(start, newRootItems, 0, Constants.RRB_BRANCHING);
            CopyTo(start + Constants.RRB_BRANCHING, newTailItems, 0, tailCount);

            var newRoot = new LeafNode<T>(newRootItems, Constants.RRB_BRANCHING, OwnerId.None);
            return new RrbList<T>(newRoot, newTailItems, count, 0, tailCount);
        }

        // 2. Prepare Tree Slice
        int treeSize = Count - TailLen;
        ...
        if (start < treeSize)
        {
            int sliceTreeEnd = Math.Min(start + count, treeSize);
            
            // Call the Optimized Recursive Slicer
            var result = SliceRecursive(Root!, Shift, 0, treeSize, start, sliceTreeEnd);
            
            newTreeRoot = result.Node;
            newTreeCount = result.Size;

            // Normalize Height (Root Collapse)
            if (newTreeRoot != null)
            {
                while (newShift > 0 && newTreeRoot is InternalNode<T> inode && inode.Len == 1)
                {
                    newTreeRoot = inode.Children[0];
        ...
        {
            // Slice extends into existing Tail
            int tailStart = Math.Max(0, start - treeSize);
            int tailEnd = (start + count) - treeSize;
            newTailLen = tailEnd - tailStart;
            newTailArray = new T[newTailLen];
            Array.Copy(Tail, tailStart, newTailArray, 0, newTailLen);
        }
        else
        {
            // Promote Tail from Tree
            if (newTreeRoot == null) return Empty;

            var (promotedNode, promotedLeaf) = RrbAlgorithm.PromoteTail(newTreeRoot, newShift, OwnerId.None);
            newTreeRoot = promotedNode;
            newTailArray = promotedLeaf.Items;
            newTailLen = promotedLeaf.Len;
            newTreeCount -= newTailLen; // Adjust count since we removed the tail

            // Re-check root collapse after promotion
            if (newTreeRoot == null) newShift = 0;
            else
            {
                while (newShift > 0 && newTreeRoot is InternalNode<T> inode && inode.Len == 1)
                {
        ...
        return new RrbList<T>(newTreeRoot, newTailArray, count, newShift, newTailLen);
    }

    /// <summary>
    /// Recursive slicer that returns (Node, Size).
    /// Prevents O(N) size recalculations by propagating sizes bottom-up.
    /// </summary>
    private (Node<T>? Node, int Size) SliceRecursive(Node<T> node, int shift, int nodeStart, int nodeEnd, int sliceStart, int sliceEnd)
    {
        // 1. Check Intersection (Fast Discard)
        if (nodeEnd <= sliceStart || nodeStart >= sliceEnd)
        {
            return (null, 0);
        }

        // 2. Full Containment (Structural Sharing - Zero Copy)
        if (nodeStart >= sliceStart && nodeEnd <= sliceEnd)
        {
            // We know the size of this node is (nodeEnd - nodeStart)
            return (node, nodeEnd - nodeStart);
        }

        // 3. Partial Leaf Slice
        if (shift == 0)
        {
            var leaf = Unsafe.As<LeafNode<T>>(node);
            int offset = Math.Max(0, sliceStart - nodeStart);
            int len = Math.Min(leaf.Len, sliceEnd - nodeStart) - offset;
            
            var newItems = new T[len];
            Array.Copy(leaf.Items, offset, newItems, 0, len);
            return (new LeafNode<T>(newItems, len, OwnerId.None), len);
        }

        // 4. Internal Node Recursive Reconstruction
        var internalNode = Unsafe.As<InternalNode<T>>(node);
        
        // Use a pooled array or stack buffer if possible, but for simplicity/safety standard array here.
        // Allocation here is amortized O(Log N).
        var newChildren = new Node<T>[internalNode.Len];
        var newSizes = new int[internalNode.Len]; // We need to build a SizeTable if we are ragged
        int newLen = 0;
        int cumulativeSize = 0;

        int currentStart = nodeStart;
        int childShift = shift - Constants.RRB_BITS;
        int denseStep = 1 << shift;

        for (int i = 0; i < internalNode.Len; i++)
        {
            // --- THE CRITICAL OPTIMIZATION ---
            // Determine Child End *without* calling GetTotalSize recursively
            int childEnd;
            if (internalNode.SizeTable != null)
            {
                // Relaxed: Read table
                childEnd = internalNode.SizeTable[i] + nodeStart;
            }
            else
            {
                // Dense: Math. 
                // Note: The last child of a dense node extends to the end of the parent.
                if (i < internalNode.Len - 1) childEnd = currentStart + denseStep;
                else childEnd = nodeEnd; // <--- FREE! No CountTree calls.
            }
            // ---------------------------------

            // Only recurse if overlap exists
            if (currentStart < sliceEnd && childEnd > sliceStart)
            {
                // Pass calculated bounds down
                var result = SliceRecursive(internalNode.Children[i]!, childShift, currentStart, childEnd, sliceStart, sliceEnd);
                
                if (result.Node != null)
                {
                    newChildren[newLen] = result.Node;
                    cumulativeSize += result.Size;
                    newSizes[newLen] = cumulativeSize;
                    newLen++;
                }
            }

            currentStart = childEnd;
            if (currentStart >= sliceEnd) break;
        }

        if (newLen == 0) return (null, 0);

        // 5. Shrink and Return
        // We always produce a Relaxed Node (with SizeTable) if we sliced anything internal, 
        // because the indices are likely shifted.
        
        var finalChildren = new Node<T>[newLen];
        Array.Copy(newChildren, finalChildren, newLen);

        var finalTable = new int[newLen];
        Array.Copy(newSizes, finalTable, newLen);

        return (new InternalNode<T>(finalChildren, finalTable, newLen, OwnerId.None), cumulativeSize);
    }
    /**
     * <summary>
